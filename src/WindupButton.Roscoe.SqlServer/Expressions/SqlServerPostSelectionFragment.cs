...
{
    public class SqlServerPostSelectionFragment : IDbFragment
    {
        private readonly string name;

        public SqlServerPostSelectionFragment(string name)
        {
            this.name = name;
        }
        ...
        public void Build(DbCommandBuilder builder, IServiceProvider serviceProvider)
        {
            ...
            if (aliasOption.EmitWithAlias)
            {
                builder.SqlBuilder.Write(" as ");
                builder.SqlBuilder.Write(name);
            }
        }
    }
}
