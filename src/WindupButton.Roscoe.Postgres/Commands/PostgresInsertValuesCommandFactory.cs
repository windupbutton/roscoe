...
{
    public class PostgresInsertValuesCommandFactory : IPostgresInsertValuesCommandFactory
    {
        public PostgresInsertValuesCommand<T> Create<T>(IServiceProvider serviceProvider, Table table, T expression)
        {
            return new PostgresInsertValuesCommand<T>(serviceProvider, table, expression);
        }
